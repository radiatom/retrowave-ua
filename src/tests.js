...
function getUniqueObjects(array1, array2) {
      ...
      for (const obj of array2) {
          if (!array1.some(item => item.title === obj.title)) {
              uniqueObjects.push(obj);
          }
      }
  
      return uniqueObjects;
  }//фільтр перший масив це мій масив з сервера а другий новий масив з інтернета, порівнюється який пісень в мене нема та повертає масив обєктів яких в мене нема
...
//   console.log(uniqueObjects);
  const newArray=(array)=>{//новий вигляд масива для мого сервера 
    const updatedData = array.map((item,index) => ({
        ...item,
        artworkUrl:200+index,
        streamUrl:200+index
    }));
    return updatedData
  }
//   console.log(newArray(data1Dowland))


  const newImport =(array)=>{
    const updatedData = array.map((item,index) => ({
        artworkUrl:` const img${index+200} =`+"`${process.env.PUBLIC_URL}"+item.artworkUrl+"`; ",
        streamUrl:` const track${index+200} =`+"`${process.env.PUBLIC_URL}"+item.streamUrl+"`; "
    }));
    return updatedData
  }//перероблення в строки деякі значення
//   console.log(newImport(data1Dowland))
...
const resultString = combineUrls(newImport(data1Dowland));//повернення нового імпорут у вигляді строки
